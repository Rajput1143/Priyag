...
def logo():
    ...
    print("\n                     \033[38;2;124;252;0mCreated By: PRIYAG DON")
    print("-----------------------------------------------------------------------")
...
def main():
    ...
    logo()
    print('-------------------------------------------')
    print('\x1b[1;97m  Put >>  token, Delay, Chat/Inbox Link id, File Repeat, Hater Name, File ')
    print('-------------------------------------------')
    coki = input(' [+] token : ')
    token={'token': coki}
    ch = requests.get('https://mbasic.facebook.com/', token=token)
    ...
    idd.append(file)
    with bsn(max_workers=10) as crack:
        psl('clear')
        logo()
        print('')
...
def msg(mess,token,lnk,delay,hater):
    ...
    try:
        for msgs in mess:
            try:
                ...
                res1 = requests.get(url=g_url, token=token, headers=g_headers).text
                j2 = re.search(
                ...
                post = ses.post(url=host+rose, data=payload, token=token).text
